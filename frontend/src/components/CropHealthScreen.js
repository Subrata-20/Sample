...
const CropHealthScreen = ({ onBack }) => {
  ...
  const takePhoto = async () => {
    ...
    try {
      const result = await ImagePicker.launchCameraAsync({
        mediaTypes: ImagePicker.MediaType.IMAGE, // Fixed deprecated MediaTypeOptions
        allowsEditing: true,
        aspect: [1, 1],
        quality: 0.8,
        base64: true,
 ...
 const analyzeCrop = async (imageAsset) => {
  try {
    setIsAnalyzing(true);
    setDiagnosis('');

    const imageToAnalyze = imageAsset || selectedImage;
    if (!imageToAnalyze || !imageToAnalyze.uri) {
    ...
    formData.append('location', 'Bihar, India'); // replace with GPS later

    const response = await api.post('/analyze_crop_disease', formData);

    if (response.data.status === 'success') {
      const report = response.data.report;
      const diagnosisText = typeof report === 'object'
        ? Object.entries(report).map(([key, value]) => `${key}:\n${value}`).join('\n\n')
        : String(report);
      setDiagnosis(diagnosisText);
    } else {
      setDiagnosis(`(Error: ${response.data.message || 'Unknown error'})`);
    }
  ...
  try {
  await api.get('/'); // Test the base URL
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        ...
        {/* Manual Analyze Button (only show if not auto-analyzing) */}
